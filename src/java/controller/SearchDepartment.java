...
public class SearchDepartment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SearchDepartment</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SearchDepartment at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Department> rList = null;
        switch (cond) {
            case "all":
                break;
            case "did":
                rList = UsersDB.search(DepDB.getAll(), s -> String.valueOf(s.getDid()).contains(txt));
                break;
            case "dname":
                rList = UsersDB.search(DepDB.getAll(), s -> s.getDname().contains(txt));
                break;
            default:
                rList = DepDB.getAll();
        }
        if (cond.equals("all")) {
            ...
            request.setAttribute("MS", "Search Department");
            request.getRequestDispatcher("listDep.jsp").forward(request, response);
        }
    }
