...
public class SearchJob extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Job> rList = null;
        switch (cond) {
            case "all":
                break;
            case "id":
                rList = UsersDB.search(JobDB.getAll(), s -> String.valueOf(s.getJid()).contains(txt));
                break;
            case "name":
                rList = UsersDB.search(JobDB.getAll(), s -> s.getJobName().contains(txt));
                break;
            case "min":
                rList = UsersDB.search(JobDB.getAll(), s -> String.valueOf(s.getMinsalary()).contains(txt));
                break;
            case "max":
                rList = UsersDB.search(JobDB.getAll(), s -> String.valueOf(s.getMaxsalary()).contains(txt));
                break;
            default:
                rList = JobDB.getAll();
        }
        if (cond.equals("all")) {
            ...
            request.setAttribute("MS", "Search Job");
            request.getRequestDispatcher("listJob.jsp").forward(request, response);
        }
    }
