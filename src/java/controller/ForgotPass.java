...
public class ForgotPass extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String ldpassword = request.getParameter("pass");
        String newpassword = request.getParameter("pass1");
        String Cpassword = request.getParameter("confirm");
        if (fullname != null && email != null && ldpassword != null && newpassword != null &&Cpassword!=null ) {
            if(!"".equals(fullname) && !"".equals(email) && !"".equals(ldpassword) && !"".equals(newpassword) &&!"".equals(Cpassword)){
                if(newpassword.equals(Cpassword)){
                    UsersDB.updatePass(fullname, email, newpassword);
                    request.setAttribute("msq", "Reset pass success");
                    request.getRequestDispatcher(response.encodeURL("login.jsp")).forward(request, response);
                }
            }else{
              request.setAttribute("msq", "Can't be left blank.");
            request.getRequestDispatcher(response.encodeURL("addnew.jsp")).forward(request, response);
        }
            
        }else{
              request.setAttribute("msq", "Reset pass faile");
            request.getRequestDispatcher(response.encodeURL("addnew.jsp")).forward(request, response);
        }
    }
